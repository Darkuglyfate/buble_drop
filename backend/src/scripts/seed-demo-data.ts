...
const logger = new Logger('BubbleDropDemoSeed');
...
type DemoSeasonSeed = Pick<
  Season,
  'key' | 'title' | 'startDate' | 'endDate' | 'isActive'
>;

export function buildDemoSeasons(seedNow = new Date()): DemoSeasonSeed[] {
  return [
    {
      key: 'genesis-bloom',
      title: 'Genesis Bloom',
      startDate: getUtcDateKey(buildDateAtUtcOffset(seedNow, -7, 0, 0, 0)),
      endDate: getUtcDateKey(buildDateAtUtcOffset(seedNow, 30, 0, 0, 0)),
      isActive: true,
    },
    {
      key: 'testnet-waves',
      title: 'Testnet Waves',
      startDate: getUtcDateKey(buildDateAtUtcOffset(seedNow, -60, 0, 0, 0)),
      endDate: getUtcDateKey(buildDateAtUtcOffset(seedNow, -1, 0, 0, 0)),
      isActive: false,
    },
  ];
}
...
export async function seedDemoData(
  ...
  if (
    ...
    options.allowProductionForTests !== true
  ) {
  ...
  try {
    ...
    const seasons = await seasonRepository.find({
      where: demoSeasons.map((season) => ({ key: season.key })),
    });
    ...
    if (!activeSeason) {
      throw new Error('Active demo season was not persisted.');
    }
    ...
    logger.log(`Seeded seasons: ${demoSeasons.length}`);
